...
class MatchScorer:
	...
	def lineups_to_array(self, match_lineups: List[MatchLineup]) -> Tuple[jnp.array, jnp.array, jnp.array, jnp.array]:
		home_players = []
		home_scores = []
		away_players = []
		away_scores = []
		for lineup in match_lineups:
			...
			home_players.append([self.player_id_map[p.player_id] for p in lineup.home_team.lineup][0:11])
			away_players.append([self.player_id_map[p.player_id] for p in lineup.away_team.lineup][0:11])
...
mcmc.run(random.PRNGKey(0),home_players, away_players,len(scorer.player_id_map), home_scores, away_scores)

predictor = Predictive(player_score_model, posterior_samples=mcmc.get_samples())

preds = predictor(random.PRNGKey(0), home_players=home_players, away_players=away_players, N_players=len(scorer.player_id_map))
