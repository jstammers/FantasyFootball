...
def iterate_stats(input_dir, output_dir):
    ...
    match_results = set(
        ["_".join(k.split("_")[2:]) for k in files if "match_results" in k]
    )
    ...
    for player_stat in player_stats:
        ...
        save_csv(
            player_df, output_dir / "player" / f"{player_stat.replace('_player', '')}"
        )
    ...
    for match_log in match_logs:
        ...
        save_csv(
            match_df,
            output_dir / "match_log" / f"{match_log.replace('_match_log', '')}",
        )
