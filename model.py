...
def ScoreModel(home_attack, home_defence, away_attack, away_defence, home_advantage,
               score_mixing, scores=(None, None)):
	home_intensity = jnp.exp(home_attack + away_defence + home_advantage)
	away_intensity = jnp.exp(away_attack + home_defence)
	home_score = npyr.sample("l1", dist.Poisson(home_intensity + score_mixing), obs=scores[0])
	away_score = npyr.sample("l2", dist.Poisson(away_intensity + score_mixing), obs=scores[1])
...
def PairwiseProbModel(score_matrix: jnp.ndarray, **options):
	...
	eta = jnp.ones(1,**options)  # Implies a uniform distribution over correlation matrices
	...
	for i, h in enumerate(alphas):
		for j, aw in enumerate(alphas):
			if i == j:
				...
				ScoreModel(h[0], h[1], aw[0], aw[1], delta, gamma,
				           [score_matrix[i, j], score_matrix[j, i]])
	return alphas

def player_score_model(home_players, away_players, N_players:int, home_score=None, away_score=None):
