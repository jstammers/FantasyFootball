...
def ingest(
    ...
    backfill_season_stats: bool = typer.Option(
        False, help="Backfill season stats"
    ),
):
    """Ingest the source data"""
    try:
        ingest_main(fb=fb, wf=wf, backfill_wf=backfill_wf, update_current_season=update_current_season,
